...
def Ks_calculator(ivector,name_analysis,sigma,error):
    ...
    for sim in ivector:
        ...
        if iresults[iresults.shape[0]-1,0] >= 2.5e4:
            print(sim)
            print('here')
    ...
    for ci in CC:
        A = sigma*ci**(0.0442)
        s1e6_dist.append(A*(1e6)**(-0.0442)) # stress distribution for 1 million cycles 
    ...
    surface_factor =  s1e6/(485.9*1.e6**(-0.0442))
    surface_factor_pos =  s1e6_pos/(485.9*1.e6**(-0.0442))
    surface_factor_neg =  s1e6_neg/(485.9*1.e6**(-0.0442))

    return surface_factor, surface_factor_pos, surface_factor_neg, me_por