...
!     enforcing Karush-Kuhn-Tucker conditions
     ...
     1 dvol*(matmul(transpose(b),stress)*((1.d0-phin)**2+xkap))       
...
! output
       UserVar(jelem,1:4,kintk)=statevLocal(1:4)*((1.d0-phin)**2+xkap)
       UserVar(jelem,5:10,kintk)=statevLocal((ntens+1):(2*ntens+2))
       UserVar(jelem,(2*ntens+3),kintk)=cL
       UserVar(jelem,(3*ntens),kintk)=H
