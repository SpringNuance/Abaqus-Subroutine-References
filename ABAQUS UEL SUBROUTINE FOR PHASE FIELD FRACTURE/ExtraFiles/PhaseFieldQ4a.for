...
! Linear quadrilateral elements, implicit integration.
...
! University of Cambridge
     ...
     3 ddlmag(mdload,*),predef(2,npredf,nnode),lflags(*),jprops(*)
      ...
      parameter(ndim=2,ntens=4,ninpt=4,nsvint=8)
...
!     enforcing Karush-Kuhn-Tucker conditions      
       Hn=UserVar(jelem,9,kintk)
       if (Psi.gt.Hn) then
        H=Psi
       else
        H=Hn
       endif        
       UserVar(jelem,9,kintk)=H   
...
! output
       UserVar(jelem,1:4,kintk)=statevLocal(1:ntens)*((1.d0-phi)**2+xk)
       UserVar(jelem,5:(2*ntens),kintk)=statevLocal((ntens+1):(2*ntens))
       UserVar(jelem,10,kintk)=phi
